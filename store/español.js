export default {
    navbar: {
        link2: "¿Quiénes somos?",
        link3: "Objetivos",
        link4: "Donar"
    },
    footer: {
        Titulo: "POLITICAS DE USO Y DATOS",
        Texto: "De conformidad con las leyes de protección de datos personales, se informa que este sitio web no obtiene ni almacena ningún tipo de información del usuario.",
        derechos: "2023 Todos los derechos reservados."
    },
    inicio: {
        titulo: "¡Sabias que puedes hacer un gran cambio!",
        subtitulo: "Ayúdanos a hacer realidad nuestro ",
        subtituloNegrita: "sueño"
    },
    about: {
        info1Titulo: "¿Es importante saberlo?",
        info1Subtitulo: "Todas las personas tenemos sueños, objetivos de vida, que en muchas ocasiones se ven frustrados por situaciones que se escapan de nuestras manos, sé que también tienes planes de vida y por lo general siempre giran en torno a 3 razones importantes:",
        card2Titulo: "Crecimiento",
        card2Info: "Los objetivos siempre buscan un crecimiento, económico, social o personal, pero es algo que nos brinda una sensación de avance.",
        card3Titulo: "Futuro",
        card3Info: "Siempre miran a un futuro próspero que no siempre se tratan de riqueza, puede ser de tranquilidad y felicidad.",
        card4Titulo: "Libertad",
        card4Info: "Pero lo más importante es la libertad, poder ser el dueño de cada minuto de vida que se va y no regresa."
    },
    goals: {
        info2titulo: "¿Cuál es la finalidad de todo esto?",
        info2Parrafo1: "Esta es mi forma de llegar a muchas personas, de manera clara y sencilla para decirles que tenemos algo en común, un gran sueño, que poco a poco avanza y está cada vez más cerca, trabajo duro para lograrlo, pero sé que hay muchos que sin condición y sin esperar nada a cambio pueden y quieren ayudarme a cumplirlo.",
        info2Parrafo2: "Quizá muchos de los que visiten este sitio web, ya lograron muchos de sus objetivos de vida, se que les habría encantado poder recibir una pequeña ayuda, hoy puedes colaborar con un soñador como tú...",
        info2Expresion: "¡No hay monto mínimo ni máximo!",
        info2Boton: "Quiero colaborar"
    },
    donate: {
        info3Titulo: "¡Lo hicimos fácil para ti!",
        info3Texto: "Muchas opciones de pago pueden ser inseguras o complicadas así que buscamos las dos más sencillas y confiables, selecciona la que quieras.",
    },
    btcP4y: {
        cargand0: "cargando...",
        p4gandoT1tulo: "Escanea o copia la dirección de pago",
        p4gandoP1e: "Esperando la confirmación en la red",
        p4gandoB0ton: "Cancelar y salir",
        recib0T1tulo: "¡Gracias por tu Donación!",
        recib0M0nto: "Se han recibido",
        recib0B0ton: "salir"
    }
}